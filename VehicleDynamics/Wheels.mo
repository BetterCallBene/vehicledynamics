...
package Wheels "Wheel, tyre and road models"
  ...
  package ParameterSets "Sub package for all parameter sets for wheel implementations"
    extends Modelica.Icons.Library2;
    record Wheel "Standard properties of a wheel"
    ...
    package RillTyre "Parameter sets for rill tyre implementation"
      extends Modelica.Icons.Library2;
      ...
      record Wheel "Tyre data"
        extends Wheels.ParameterSets.Wheel;
        extends Modelica.Icons.Record;
        ...
        VehicleDynamics.Wheels.ParameterSets.RillTyre.NominalLoad load2(Fz_nom = 6000, Fds_x = 75000, s_max_x = 0.18, F_max_x = 4500, s_slide_x = 0.5, F_slide_x = 4200, Fds_y = 60000, s_max_y = 0.24, F_max_y = 4125, s_slide_y = 0.8, F_slide_y = 3750, dL0 = 0.1, s0 = 0.275, sE = 0.8) annotation(
  ...
  partial model BaseWheel "Properties common to all types of wheels and tyres"
    ...
    replaceable ParameterSets.Wheel wheelData;
    // Modify deprecated VehicleDynamics Library
...
// Collect all connectors
    ...
    annotation(
      defaultComponentName="rill tyre", 
      Icon(coordinateSystem(
          ...
          Text(
            ...
            lineColor={0,0,255})}),
      Diagram,
      Documentation(info = "<html><head></head><body><div>This element is a superclass of wheel/tyre</div><div>systems. It contains all the equations which are independent of the</div><div>tyre model. Especially, the contact point of wheel and road, as well</div><div>as the contact velocities are computed.</div><div>The frames of this object have the following meaning:</div><div>Frame carrierFrame is fixed in the carrier of the wheel</div><div>and is located on the spin axis of the wheel in the</div><div>middle point of the wheel. This point is also approximately</div><div>used as the center-of-mass of the wheel.</div><div>It is temporarily assumed that the road lies in the</div><div>x-y plane of the inertial frame! This restriction can</div><div>be removed by defining the road properties via inner/outer</div><div>functions.</div><div>Flange XXX is used to drive the wheel with a 1D-drive train.</div><div>This element is realized according to the description:</div><div>&nbsp; Georg Rill: Simulation von Kraftfahrzeugen, Vieweg, 1994</div></body></html>"));
  end BaseWheel;
  ...
  package RillTyre
    ...
    block TyreForces "Compute tyre forces and torques for steady state conditions"
      ...
      //parameter ParameterSets.RillTyre.Wheel data "Data of tyre";
      VehicleDynamics.Wheels.ParameterSets.RillTyre.Wheel data annotation(
...
// Compute attachment point of f_y with respect to contact length (dL)
    ...
    */
    ...
    //and roll resistance torque.
      ...
      package Tests "Tests tyre implementation"
              ...
              model BasicCarFourWheels
        Modelica.Mechanics.MultiBody.Parts.Body body(I_11 = 1, I_22 = 1, I_33 = 1, enforceStates = true, m = 200, r_0(start = {0, 0, 0.31}), r_CM = {0, 0, 0}) annotation(
          Placement(visible = true, transformation(origin = {-12, 50}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      VehicleDynamics.Wheels.RillTyre.Wheel wheel annotation(
          Placement(visible = true, transformation(origin = {0, 8}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      VehicleDynamics.Wheels.RillTyre.Wheel wheel1(leftWheel = false)  annotation(
          Placement(visible = true, transformation(origin = {-54, 16}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation2(r = {2, 2, 0})  annotation(
          Placement(visible = true, transformation(origin = {50, 24}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation(r = {2, -2, 0})  annotation(
          Placement(visible = true, transformation(origin = {-60, 50}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
      Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation1(r = {-2, 2, 0})  annotation(
          Placement(visible = true, transformation(origin = {-58, 78}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation3(r = {-2, -2, 0})  annotation(
          Placement(visible = true, transformation(origin = {12, 82}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      RillTyre.Wheel wheel2 annotation(
          Placement(visible = true, transformation(origin = {50, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      RillTyre.Wheel wheel3 annotation(
          Placement(visible = true, transformation(origin = {-28, 78}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.Ramp ramp(duration = 3, height = 2, startTime = 4)  annotation(
  ...
  Modelica.Mechanics.Rotational.Sources.Torque torque annotation(
      ...
      equation
        connect(body.frame_a, fixedTranslation2.frame_a) annotation(
          Line(points = {{-22, 50}, {40, 50}, {40, 24}}));
        connect(body.frame_a, fixedTranslation.frame_a) annotation(
          Line(points = {{-22, 50}, {-50, 50}}, color = {95, 95, 95}));
        connect(wheel1.carrierFrame, fixedTranslation.frame_b) annotation(
          Line(points = {{-64, 16}, {-64, 34}, {-70, 34}, {-70, 50}}, color = {95, 95, 95}));
        connect(wheel.carrierFrame, fixedTranslation2.frame_b) annotation(
          Line(points = {{-10, 8}, {60, 8}, {60, 24}}));
        connect(body.frame_a, fixedTranslation3.frame_a) annotation(
          Line(points = {{-22, 50}, {2, 50}, {2, 82}, {2, 82}}, color = {95, 95, 95}));
        connect(body.frame_a, fixedTranslation1.frame_a) annotation(
          Line(points = {{-22, 50}, {-68, 50}, {-68, 78}, {-68, 78}}));
      connect(fixedTranslation1.frame_b, wheel3.carrierFrame) annotation(
          Line(points = {{-48, 78}, {-38, 78}, {-38, 78}, {-38, 78}, {-38, 78}}, color = {95, 95, 95}));
      connect(fixedTranslation3.frame_b, wheel2.carrierFrame) annotation(
          Line(points = {{22, 82}, {42, 82}, {42, 80}, {40, 80}, {40, 80}}));
  connect(ramp.y, torque.tau) annotation(
        Line(points = {{-50, -18}, {-42, -18}, {-42, -22}, {-36, -22}}, color = {0, 0, 127}));
  connect(torque.flange, wheel1.driveShaft1D) annotation(
        Line(points = {{-14, -22}, {-54, -22}, {-54, 26}, {-54, 26}}));
  connect(torque.flange, wheel.driveShaft1D) annotation(
        Line(points = {{-14, -22}, {0, -22}, {0, 18}, {0, 18}}));
      end BasicCarFourWheels;
              model BasicCarFourWheelsTest
        VehicleDynamics.Wheels.Tests.BasicCarFourWheels basicCarFourWheels annotation(
